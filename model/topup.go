...
type TopUp struct {
	...
	InviterRewardSent bool    `json:"inviter_reward_sent" gorm:"default:false"` // custom: invite rebate (PR #3495)
}
...
const (
	...
	PaymentProviderWallet       = "wallet" // custom: wallet subscription
)
...
func ManualCompleteTopUp(tradeNo, callerIp string) (ManualCompleteResult, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.Status != common.TopUpStatusPending {
			return fmt.Errorf("订单状态不是待支付(%s)，无法补单", topUp.Status)
		}
